# Test default parameters
test_that("fsrs_default_parameters returns 21 parameters", {
  params <- fsrs_default_parameters()
  expect_length(params, 21)
  expect_type(params, "double")
  expect_true(all(params >= 0))
})

# Test initial state
test_that("fsrs_new_card_state returns valid state", {
  # Test all ratings
  for (rating in 1:4) {
    state <- fsrs_new_card_state(rating = rating)
    expect_type(state, "list")
    expect_named(state, c("stability", "difficulty"))
    expect_true(state$stability > 0)
    expect_true(state$difficulty >= 1 && state$difficulty <= 10)
  }
})

test_that("fsrs_new_card_state stability increases with rating", {
  state_again <- fsrs_new_card_state(rating = 1)
  state_hard <- fsrs_new_card_state(rating = 2)
  state_good <- fsrs_new_card_state(rating = 3)
  state_easy <- fsrs_new_card_state(rating = 4)

  expect_lt(state_again$stability, state_hard$stability)
  expect_lt(state_hard$stability, state_good$stability)
  expect_lt(state_good$stability, state_easy$stability)
})

# Test retrievability
test_that("fsrs_retrievability returns valid probability", {
  stability <- 2.5

  # Retrievability should be between 0 and 1
  r <- fsrs_retrievability(stability, elapsed_days = 1)
  expect_gte(r, 0)
  expect_lte(r, 1)

  # Retrievability should decrease over time
  r1 <- fsrs_retrievability(stability, elapsed_days = 1)
  r7 <- fsrs_retrievability(stability, elapsed_days = 7)
  r30 <- fsrs_retrievability(stability, elapsed_days = 30)

  expect_gt(r1, r7)
  expect_gt(r7, r30)
})

test_that("fsrs_retrievability at day 0 is ~1", {
  stability <- 5.0
  r <- fsrs_retrievability(stability, elapsed_days = 0)
  expect_equal(r, 1, tolerance = 0.001)
})

test_that("fsrs_retrievability at stability days is ~0.9", {
  stability <- 10.0
  # By definition, stability is time until R drops to 90%
  r <- fsrs_retrievability(stability, elapsed_days = stability)
  expect_equal(r, 0.9, tolerance = 0.01)
})

# Test next state
test_that("fsrs_next_memory_state returns valid state", {
  initial <- fsrs_new_card_state(rating = 3)

  new_state <- fsrs_next_memory_state(
    stability = initial$stability,
    difficulty = initial$difficulty,
    elapsed_days = 1,
    rating = 3
  )

  expect_type(new_state, "list")
  expect_named(new_state, c("stability", "difficulty"))
  expect_true(new_state$stability > 0)
  expect_true(new_state$difficulty >= 1 && new_state$difficulty <= 10)
})

test_that("fsrs_next_memory_state stability grows after review", {
  initial <- fsrs_new_card_state(rating = 3)

  # Review with "Good" rating after some days
  new_state <- fsrs_next_memory_state(
    stability = initial$stability,
    difficulty = initial$difficulty,
    elapsed_days = 1,
    rating = 3
  )

  # Stability should increase after a successful review
  expect_gt(new_state$stability, initial$stability)
})

test_that("fsrs_next_memory_state stability decreases on 'Again'", {
  # Build up some stability first
  state <- fsrs_new_card_state(rating = 3)
  state <- fsrs_next_memory_state(state$stability, state$difficulty, 3, 3)
  state <- fsrs_next_memory_state(state$stability, state$difficulty, 5, 3)

  high_stability <- state$stability

  # Now fail the card
  failed_state <- fsrs_next_memory_state(
    stability = state$stability,
    difficulty = state$difficulty,
    elapsed_days = 10,
    rating = 1  # Again
  )

  expect_lt(failed_state$stability, high_stability)
})

# Test next interval
test_that("fsrs_interval returns positive value", {
  state <- fsrs_new_card_state(rating = 3)

  # Default retention of 0.9
  interval <- fsrs_interval(state$stability, desired_retention = 0.9)

  expect_type(interval, "double")
  expect_gte(interval, 0)
})

test_that("fsrs_interval is proportional to stability", {
  # Higher stability should give higher interval for same retention
  interval_low <- fsrs_interval(stability = 2.0, desired_retention = 0.9)
  interval_high <- fsrs_interval(stability = 20.0, desired_retention = 0.9)

  expect_lt(interval_low, interval_high)
})

test_that("fsrs_interval decreases with higher retention", {
  state <- fsrs_new_card_state(rating = 3)

  int_85 <- fsrs_interval(state$stability, desired_retention = 0.85)
  int_90 <- fsrs_interval(state$stability, desired_retention = 0.90)
  int_95 <- fsrs_interval(state$stability, desired_retention = 0.95)

  # Higher desired retention = shorter intervals
  expect_gt(int_85, int_90)
  expect_gt(int_90, int_95)
})

# Integration test
test_that("review cycle produces growing stability", {
  # Simulate learning a card over multiple reviews
  state <- fsrs_new_card_state(rating = 3)

  # Track that all states are valid
  expect_true(state$stability > 0)
  expect_true(state$difficulty >= 1 && state$difficulty <= 10)

  prev_stability <- state$stability
  for (i in 1:5) {
    interval <- fsrs_interval(state$stability, desired_retention = 0.9)
    state <- fsrs_next_memory_state(state$stability, state$difficulty,
                                    elapsed_days = max(1, interval), rating = 3)

    # Each state should be valid
    expect_true(state$stability > 0)
    expect_true(state$difficulty >= 1 && state$difficulty <= 10)
    # Stability should grow with successful reviews
    expect_gt(state$stability, prev_stability)
    prev_stability <- state$stability
  }
})

test_that("easy ratings produce higher stability than hard ratings", {
  state <- fsrs_new_card_state(rating = 3)

  # Review with different ratings
  state_hard <- fsrs_next_memory_state(state$stability, state$difficulty, elapsed_days = 2, rating = 2)
  state_easy <- fsrs_next_memory_state(state$stability, state$difficulty, elapsed_days = 2, rating = 4)

  expect_gt(state_easy$stability, state_hard$stability)
})
